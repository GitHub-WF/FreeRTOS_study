...
uint32_t bitArrs[] = {1 << 0, 1 << 1, 1 << 2, 1 << 3};
void task1(void *arg)
{
  ...
  while (1)
  {
    if (key_is_pressed != 0)
    {
      printf("press key 1\r\n");
      /* 发送任务通知，任务通知模拟时间标志组 */
      xTaskNotify(task2_handle, bitArrs[key_is_pressed - 1], eSetBits);
      xTaskNotifyIndexed(task2_handle, 0, bitArrs[key_is_pressed - 1], eSetBits); // 和xTaskNotifyWaitIndexed搭配使用
...
void task2(void *arg)
{
  ...
  while (1)
  {
    ...
    res = xTaskNotifyWait(
        0, // 接收通知前：0x00000000 32位都不清除 0xffffffff 32位都清除
        0xffffffff, // 接收通知后：同上
        &notifyVal,
        portMAX_DELAY);
    if (res == pdTRUE)
